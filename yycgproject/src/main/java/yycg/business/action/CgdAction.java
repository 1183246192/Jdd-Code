...
import yycg.business.dao.mapper.SysSeqMapper;
...
public class CgdAction {
	...
	public @ResponseBody SubmitResultInfo yycgdmxdelsubmit(//
			String id,// 采购单id
			YycgdQueryVo yycgdQueryVo, //
			int[] indexs // 页面选择序号
	) throws Exception {
		List<YycgdmxCustom> yycgdmxCustoms = cgdService.findYycgdmxListByYycgdid(id, null);// 数据库中的信息
		List<YycgdmxCustom> list = yycgdQueryVo.getYycgdmxCustoms();// 网页传过来的数据
		...
		for (int i = 0; i < count; i++) {
			YycgdmxCustom yycgdmxCustom_y = list.get(indexs[i]);
			if (yycgdmxCustom_y != null) {
				String yycgdmx_yid = yycgdmxCustom_y.getYycgdmxid();
				for (YycgdmxCustom yycgdmxCustom_s : yycgdmxCustoms) {
					String yycgdmx_sid = yycgdmxCustom_s.getYycgdmxid();
					if (yycgdmx_sid.equals(yycgdmx_yid)) {
						try {
							...
							criteria.andIdEqualTo(yycgdmx_yid);
		...
		return ResultUtil.createSubmitResult(
				ResultUtil.createSuccess(Config.MESSAGE, 906, new Object[] { count_success, count_error }), msgs_error);
	}
	...
	public @ResponseBody DataGridResultInfo yycgdlist_result(HttpSession session, String year,// 年份
			YycgdQueryVo yycgdQueryVo,// 查询条件
			int page, int rows) throws Exception {
	...
	public @ResponseBody DataGridResultInfo checkyycgdlist_result(HttpSession session, String year,// 年份
			YycgdQueryVo yycgdQueryVo,// 查询条件
			int page, int rows) throws Exception {
		ActiveUser activeUser = (ActiveUser) session.getAttribute(Config.ACTIVEUSER_KEY);
		// 监管单位id
