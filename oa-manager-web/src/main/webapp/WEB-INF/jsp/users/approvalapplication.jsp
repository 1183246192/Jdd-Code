...
//		width : 20
	...
	var toolbar_v = [{
		...
		handler: function(){
        	...
        	if(ids.length == 0){
        		$.messager.alert('提示','必须选择一个用户才能审批!','info');
        		return ;
        	}
//			alert(ids.indexOf(','));
        	if(ids.length > 1){
 ...
 //       	console.info(ids);
			data = ids[0];
			createmodalwindow("审核休假申请", 800, 320, '${baseurl}users/editapproval?id='+data.id);
		}
	}];
	...
	$(function() {
		$('#applicationlist').datagrid({
			...
			striped : true,//条纹显示效果
//			singleSelect : true,
			url : '${baseurl}users/queryapproval_result',//加载数据的连接，引连接请求过来是json数据
	...
	function queryvacation(){
		...
		var formdata = $("#approvalqueryForm").serializeJson();
		$('#applicationlist').datagrid('load',formdata);
	}
	
	//清空查询条件
	function clearSearchConditionForm(){
		$('#approvalqueryForm').form('clear');
		queryvacation();
	}
